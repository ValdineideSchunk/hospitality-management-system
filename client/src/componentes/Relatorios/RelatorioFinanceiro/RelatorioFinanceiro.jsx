...
ChartJS.register(
    ...
    Tooltip,
    Legend,
    ArcElement // Registrar para o gráfico de pizza
);
...
const RelatorioFinanceiro = () => {
    ...
    const buscarRelatorio = async () => {
        try {
            ...
            // Calculando a receita total do período
    ...
    const gerarGraficoBarras = () => {
        ...
        const data = dadosRelatorio.map(d => parseFloat(d.receita_total));

        return {
            labels,
            datasets: [
                {
                    label: 'Receita Total por Acomodação (R$)',
                    data,
    ...
    return (
        <div style={{
            ...
            <h2>Relatório Financeiro por Acomodação</h2>
            ...
            {dadosRelatorio.length > 0 && (
                <div>
                    <h3>Tabela de Receita por Acomodação</h3>
                    <table className="table table-striped table-bordered mt-3">
                        ...
                        <tbody>
                            {dadosRelatorio.map((d, index) => (
                                <tr key={index}>
                                    <td>{d.fk_acomodacao}</td>
                                    <td>{d.nome_acomodacao}</td>
                                    <td>{d.total_reservas}</td>
                                    <td>{d.total_diarias_reservadas}</td>
                                    <td>{formatarMoeda(d.media_diaria)}</td>
                                    <td>{formatarMoeda(d.receita_total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <h3 className="mt-4">Receita Total do Período: {formatarMoeda(receitaTotalPeriodo)}</h3>
                    ...
                    {tipoGrafico === 'barras' ? (
                        <div>
                            <h3 className="mt-5">Gráfico de Receita por Acomodação (Barras)</h3>
                        ...
                        <div>
                            <h3 className="mt-5">Gráfico de Receita por Acomodação (Pizza)</h3>
                            <div style={{ width: '60%', margin: '0 auto' }}>
                                <Pie
